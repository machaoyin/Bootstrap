...
module.exports = function (grunt) {
  ...
  grunt.initConfig({
    ...
    concat: {
      ...
      core: {
        ...
        dest: 'summernote/js/<%= pkg.name %>.js'
      }
    },
    ...
    uglify: {
      ...
      core: {
        ...
        dest: 'summernote/js/<%= pkg.name %>.min.js'
      },
      customize: {
    ...
    less: {
      ...
      core: {
        options: {
          ...
          sourceMapFilename: 'summernote/css/<%= pkg.name %>.css.map'
        },
        src: 'less/bootstrap.less',
        dest: 'summernote/css/<%= pkg.name %>.css'
      },
      theme: {
        options: {
          sourceMapURL: '<%= pkg.name %>-theme.css.map',
          sourceMapFilename: 'summernote/css/<%= pkg.name %>-theme.css.map'
        },
        src: 'less/theme.less',
        dest: 'summernote/css/<%= pkg.name %>-theme.css'
      },
      docs: {
        options: {
    ...
    postcss: {
      ...
      core: {
        src: 'summernote/css/<%= pkg.name %>.css'
      },
      theme: {
        src: 'summernote/css/<%= pkg.name %>-theme.css'
      },
      docs: {
    ...
    cssmin: {
      ...
      core: {
        src: 'summernote/css/<%= pkg.name %>.css',
        dest: 'summernote/css/<%= pkg.name %>.min.css'
      },
      theme: {
        src: 'summernote/css/<%= pkg.name %>-theme.css',
        dest: 'summernote/css/<%= pkg.name %>-theme.min.css'
      },
      docs: {
        src: 'docs/assets/css/docs.css',
  ...
  if (runSubset('core')) {
    testSubtasks = testSubtasks.concat(['summernote-css', 'summernote-js', 'stylelint:summernote', 'test-js', 'docs']);
  }
  ...
  grunt.registerTask('summernote-js', ['concat', 'uglify:core', 'commonjs']);

  // CSS distribution task.
  grunt.registerTask('summernote-css', ['less:core', 'less:theme', 'postcss:core', 'postcss:theme', 'cssmin:core', 'cssmin:theme']);

  // Full distribution task.
  grunt.registerTask('dist', ['clean:summernote', 'summernote-css', 'copy:fonts', 'summernote-js']);

  // Default task.
  grunt.registerTask('default', ['clean:summernote', 'copy:fonts', 'test']);
  ...
  grunt.registerTask('commonjs', 'Generate CommonJS entrypoint module in summernote dir.', function () {
    var srcFiles = grunt.config.get('concat.core.src');
    var destFilepath = 'summernote/js/npm.js';
